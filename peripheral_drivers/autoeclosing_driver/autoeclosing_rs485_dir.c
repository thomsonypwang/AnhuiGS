...
static void autoeclosing_irq_handler(const stc_irq_signin_config_t *pstcConfig, uint32_t u32Priority)
{
    if (NULL != pstcConfig) 
	{
...
static void autoeclosing_timerout_config(uint16_t u16TimeoutBits)
{
    uint16_t u16Div;
    uint16_t u16Delay;    
...
void autoeclosing_usart_init(func_ptr_t func_rx_err_callback,func_ptr_t func_rx_full_callback,func_ptr_t func_rx_timeout_callback)
{
	...
	stcUartInit.u32Baudrate = AUTOCLOSING_USART_BAUDRATE;
	...
	autoeclosing_irq_handler(&stcIrqSigninConfig, DDL_IRQ_PRIO_12);
	...
	autoeclosing_irq_handler(&stcIrqSigninConfig, DDL_IRQ_PRIO_14);
	...
	autoeclosing_irq_handler(&stcIrqSigninConfig, DDL_IRQ_PRIO_13);
	
	autoeclosing_timerout_config(AUTOCLOSING_USART_TMR0_TIMEROUT);
