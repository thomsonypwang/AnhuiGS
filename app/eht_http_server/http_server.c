...
void loadwebpages(void)
{
		...
		reg_httpServer_webContent((uint8_t *)"info.js", (uint8_t *)w5500_info_js);	// netinfo.js 		: JavaScript for Read Network configuration 	(+ ajax.js)
		reg_httpServer_webContent((uint8_t *)"setinfo.js", (uint8_t *)w5500_setinfo_js);	// netinfo.js 		: JavaScript for Read Network configuration 	(+ ajax.js)	
//		reg_httpServer_webContent((uint8_t *)"setthinfo.js", (uint8_t *)w5500_setthinfo_js);	// netinfo.js 		: JavaScript for Read Network configuration 	(+ ajax.js)
//		reg_httpServer_webContent((uint8_t *)"setNetinfo.js", (uint8_t *)w5500_setNetinfo_js);	// netinfo.js 		: JavaScript for Read Network configuration 	(+ ajax.js)
//		reg_httpServer_webContent((uint8_t *)"setMqttinfo.js", (uint8_t *)w5500_setMqttinfo_js);	// netinfo.js 		: JavaScript for Read Network configuration 	(+ ajax.js)	
		// AJAX JavaScript functions
		reg_httpServer_webContent((uint8_t *)"ajax.js", (uint8_t *)w5500_ajax_js);			// ajax.js			: JavaScript for AJAX request transfer		
		//favicon.ico
		...
		case SOCK_ESTABLISHED:
			...
			switch(HTTPSock_Status[seqnum].sock_status)
			{
						...
						//else // HTTP process handler
						{
							*(((uint8_t *)http_request) + len) = '\0';	// End of string (EOS) marker
							parse_http_request(parsed_http_request, (uint8_t *)http_request);
								...
								log_i("> HTTPSocket[%d] : HTTP Request received ", s);
								log_i("from %d.%d.%d.%d : %d", destip[0], destip[1], destip[2], destip[3], destport);
							#endif

							#ifdef _HTTPSERVER_DEBUG_
								log_i("> HTTPSocket[%d] : [State] STATE_HTTP_REQ_DONE", s);
							#endif
				...
				case STATE_HTTP_RES_INPROC :
					...
					#ifdef _HTTPSERVER_DEBUG_
						log_i("> HTTPSocket[%d] : [State] STATE_HTTP_RES_INPROC", s);
					#endif
				...
				case STATE_HTTP_RES_DONE :
					#ifdef _HTTPSERVER_DEBUG_
						log_i("> HTTPSocket[%d] : [State] STATE_HTTP_RES_DONE", s);
					#endif
...
//					#endif
		...
		case SOCK_CLOSE_WAIT:
			#ifdef _HTTPSERVER_DEBUG_
				log_i("> HTTPSocket[%d] : ClOSE_WAIT", s);	// if a peer requests to close the current connection
			#endif
			// Socket file info structure re-initialize
			HTTPSock_Status[seqnum].file_len = 0;
			HTTPSock_Status[seqnum].file_offset = 0;
...
static void send_http_response_header(uint8_t s, uint8_t content_type, uint32_t body_len, uint16_t http_status)
{
	switch(http_status)
	{
		case STATUS_OK: 		// HTTP/1.1 200 OK
			...
			{
				#ifdef _HTTPSERVER_DEBUG_
					log_i("> HTTPSocket[%d] : HTTP Response Header - STATUS_OK", s);
				#endif
			...
			{
				#ifdef _HTTPSERVER_DEBUG_
					log_i("> HTTPSocket[%d] : HTTP Response Header - NONE / CGI or XML", s);
				#endif
		...
		case STATUS_BAD_REQ: 	// HTTP/1.1 400 OK
			#ifdef _HTTPSERVER_DEBUG_
				log_i("> HTTPSocket[%d] : HTTP Response Header - STATUS_BAD_REQ", s);
			#endif
			memcpy(http_response, ERROR_REQUEST_PAGE, sizeof(ERROR_REQUEST_PAGE));
			break;
		case STATUS_NOT_FOUND:	// HTTP/1.1 404 Not Found
			#ifdef _HTTPSERVER_DEBUG_
				log_i("> HTTPSocket[%d] : HTTP Response Header - STATUS_NOT_FOUND", s);
			#endif
	...
	{
		#ifdef _HTTPSERVER_DEBUG_
			log_i("> HTTPSocket[%d] : [Send] HTTP Response Header [ %d ]byte", s, strlen((char *)http_response));
		#endif
...
// ## 20141219 Eric added, for 'File object structure' (fs) allocation reduced (8 -> 1)
			...
			#ifdef _HTTPSERVER_DEBUG_
				log_i("> HTTPSocket[%d] : HTTP Response body - file name [ %s ]", s, HTTPSock_Status[get_seqnum].file_name);
				log_i("> HTTPSocket[%d] : HTTP Response body - file len [ %d ]byte", s, file_len);
			#endif
/////////////////////////////////////////////////////////////////////////////////////////////////
		}
		else
	...
	else // ### Send HTTP response body: Remained parts ###
	{
		...
		{
			#ifdef _HTTPSERVER_DEBUG_
				log_i("> HTTPSocket[%d] : HTTP Response end - file len [ %d ]byte", s, HTTPSock_Status[get_seqnum].file_len);
			#endif
			// Send process end
			flag_datasend_end = 1;
		}
		#ifdef _HTTPSERVER_DEBUG_
			log_i("> HTTPSocket[%d] : HTTP Response body - send len [ %d ]byte", s, send_len);
		#endif
	}
...
// ## 20141219 Eric added, for 'File object structure' (fs) allocation reduced (8 -> 1)
	...
	#ifdef _HTTPSERVER_DEBUG_
		log_i("> HTTPSocket[%d] : [Send] HTTP Response body [ %d ]byte", s, send_len);
	#endif
	...
	else
	{
		HTTPSock_Status[get_seqnum].file_offset += send_len;
		#ifdef _HTTPSERVER_DEBUG_
			log_i("> HTTPSocket[%d] : HTTP Response body - offset [ %d ]", s, HTTPSock_Status[get_seqnum].file_offset);
		#endif
	}
...
static void send_http_response_cgi(uint8_t s, uint8_t * buf, uint8_t * http_body, uint16_t file_len)
{
	...
	#ifdef _HTTPSERVER_DEBUG_
		log_i("> HTTPSocket[%d] : HTTP Response Header + Body - CGI", s);
	#endif
	send_len = sprintf((char *)buf, "%s%d\r\n\r\n%s", RES_CGIHEAD_OK, file_len, http_body);
	#ifdef _HTTPSERVER_DEBUG_
		log_i("> HTTPSocket[%d] : HTTP Response Header + Body - send len [ %d ]byte", s, send_len);
	#endif
	send(s, buf, send_len);
}
...
static void http_process_handler(uint8_t s, st_http_request * p_http_request)
{
	...
	switch (p_http_request->METHOD)
	{
		...
		case METHOD_GET :
			...
			#ifdef _HTTPSERVER_DEBUG_
				log_i("> HTTPSocket[%d] : HTTP Method GET", s);
				log_i("> HTTPSocket[%d] : Request Type = %d", s, p_http_request->TYPE);
				log_i("> HTTPSocket[%d] : Request URI = %s", s, uri_name);
			#endif
			...
			{// If No CGI request, Try to find The requested web content in storage (e.g., 'SD card' or 'Data flash')
				...
				{
					#ifdef _HTTPSERVER_DEBUG_
						log_i("> HTTPSocket[%d] : Unknown Page Request", s);
					#endif
				...
				{
					#ifdef _HTTPSERVER_DEBUG_
						log_i("> HTTPSocket[%d] : Find Content [%s] ok - Start [%d] len [ %d ]byte", s, uri_name, content_addr, file_len);
					#endif
				...
				{
					#ifdef _HTTPSERVER_DEBUG_
						log_i("> HTTPSocket[%d] : Requested content len = [ %d ]byte", s, file_len);
					#endif
		...
		log_i("> METHOD_POST = %s ", (char *)p_http_request->URI);
			...
			#ifdef _HTTPSERVER_DEBUG_
				log_i("> HTTPSocket[%d] : HTTP Method POST", s);
				log_i("> HTTPSocket[%d] : Request URI = %s ", s, post_name);
				log_i("Type = %d", p_http_request->TYPE);
			#endif
			...
			if(p_http_request->TYPE == PTYPE_CGI)	// HTTP POST Method; CGI Process
			{
				...
				#ifdef _HTTPSERVER_DEBUG_
					log_i("> HTTPSocket[%d] : [CGI: %s] / Response len [ %d ]byte", s, content_found?"Content found":"Content not found", file_len);
				#endif
				if(content_found && (file_len <= (DATA_BUF_SIZE-(strlen(RES_CGIHEAD_OK)+8))))
				{
...
void reg_httpServer_webContent(uint8_t * content_name, uint8_t * content)
{
	...
	web_content[total_content_cnt].content_name = os_mem_alloc(name_len+1);
...
uint8_t display_reg_webContent_list(void)
{
	...
	{
		log_i(">> Web content file not found");
		ret = 0;
	}
	else
	{
		//log_i("=== List of Web content in code flash ===");
		for(i = 0; i < total_content_cnt; i++)
		{
			//log_i(" [%d] ", i+1);
			//log_i("%s, ", web_content[i].content_name);
			//log_i("%u byte, ", web_content[i].content_len);

			if(web_content[i].content_len < 30)
			{
				//log_i("[%s]", web_content[i].content);
			}
			else
			{
			//	log_i("[ ... ]");
