...
//		4,		/* The priority assigned to the task is defined in FreeRTOSConfig.h. */
...
int FreeRTOS_read(Network* n, unsigned char* buffer, int len, int timeout_ms)
{
	...
	do
	{
		setsockopt(n->my_socket, SO_KEEPALIVEAUTO, (void*)&xTicksToWait);
...
void NetworkInit(Network* n, int sn)
{
	n->my_socket = sn;
...
int NetworkConnect(Network* n, uint8_t* addr, int port)
{
...
exit:
	...
	if(connect(n->my_socket, addr, port) != SOCK_OK)
		return SOCK_ERROR;
