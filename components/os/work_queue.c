...
#ifdef DBG_ENABLE_WQ
	#define wq_d(...)	log("wq", ##__VA_ARGS__)
	#define wq_e(...) 	log_e("wq", ##__VA_ARGS__)
	#define wq_w(...) 	log_w("wq", ##__VA_ARGS__)
	#define wq_entry(...)
	#define wq_entry_i(...)
#else
	#define wq_d(...)
...
typedef struct {
	...
	/*
	 * Single shot jobs which have already run and which have a thread
	 * waiting on them will be stored in this list.
	 */
	list_head_t wq_done_job_list;
...
static wq_job_info_t *wq_get_preallocated_job_info(wq_t *wq)
{
	...
	wq_job_info_t *wq_job_info = list_entry(wq->wq_job_info_isr_list.next,wq_job_info_t, node);
...
static int wq_prepare_job_info(wq_t *wq, const wq_job_t *wq_job, wq_job_info_t **wq_job_info, bool init_sem)
{
	*wq_job_info = NULL;
	...
	l_wq_job_info->next_exec_tick = os_total_ticks_get() + wq_job->initial_delay_ms;
			  ...
			  unsigned long timeout_ms, int *ret_status)
{
	...
	if (rv != SYS_OK) 
	{
		 ...
		 */
		os_disable_all_interrupts();
		/* Try once more to avoid race condition */
		if (os_semaphore_get(&wq_job_info->sync_sem, OS_NO_WAIT) != SYS_OK) 
		{
			/* The job has still not executed */
			os_semaphore_delete(&wq_job_info->sync_sem);
			wq_job_info->sync_sem = NULL;
			os_enable_all_interrupts();

			/*
			 * If the job completes in the background it will
			 * deque and free itself as 'sync_sem' is made NULL
			 * above thus categorizing it as a normal 'async'
			 * job.
			 */
		...
		/* The job execution is done. Lets ignore the timeout
		   occured on the edge */
	}

	/* The job has been moved to the done job list */
	if (ret_status)
...
int work_enqueue(wq_handle_t wq_handle, const wq_job_t *wq_job,job_handle_t *job_handle)
{
	wq_entry("%p", wq_job->job_func);
	wq_t *wq = (wq_t *) wq_handle;
	if (!wq || !wq_job || !wq_job->job_func ||
...
{
	wq_job_info_t *wq_job_info;
	list_for_each_entry(wq_job_info, wq_job_info_t, &wq->wq_job_list,node, struct list_head) 
	{
...
int work_dequeue(wq_handle_t wq_handle, job_handle_t *job_handle)
{
	...
	int rv;
	int sat=os_enter_critical_section();
...
bool is_owner_job_queued(wq_handle_t wq_handle, const char *owner)
{
	...
	list_for_each_entry(wq_job_info, wq_job_info_t, &wq->wq_job_list,node, struct list_head) 
	{
		...
		if (!strncmp(owner, wq_job_info->wq_job.owner,MAX_OWNER_NAME_LEN)) 
		{
			found = true;
			break;
		}
...
{
	#ifdef WQ_DUMP_CALLED_FUNCTION
		wmlog("wq", "wq thread: %s invoking %p", get_current_taskname(),wq_job_info->wq_job.job_func);
	#endif
...
static unsigned long wq_get_sleep_ticks(wq_t *wq)
{
	...
	list_for_each_entry(wq_job_info, wq_job_info_t, &wq->wq_job_list,node, struct list_head) 
	{
		...
		uint32_t remaining_ticks =wq_job_info->next_exec_tick - curr_tick;
...
static void wq_purge_job_list(wq_t *wq, bool purge_isr_list)
{
	...
	{
		/* Get the first job in the list */
		wq_job_info_t *wq_job_info = list_entry(wq->wq_job_list.next,wq_job_info_t,node);
		wq_d("%s: De-queueing: %p", __func__,wq_job_info->wq_job.job_func);
		deque_and_free_job(wq, wq_job_info);
	}
...
/*
	...
	else 
	{
		...
		if (wq_job_info->sync_sem) 
		{
			...
			/*
			 * Move this job to done queue to be
			 * freed later when this semaphore signal is
			 * successfully delivered.
			 */
			list_del(&wq_job_info->node);
			list_add_tail(&wq_job_info->node,
				      &wq->wq_done_job_list);
...
static int alloc_job_info_isr_list(wq_t *wq, int count)
{
	...
	{
		wq_job_info_t *wq_job_info =os_mem_calloc(sizeof(wq_job_info_t));
		if (!wq_job_info)
		{
			return SYS_E_NOMEM;
		}
...
int work_queue_init(const wq_cfg_t *wq_cfg, wq_handle_t *wq_handle)
{
	...
	INIT_LIST_HEAD(&wq->wq_done_job_list);
	...
	rv = os_thread_create(&wq->worker_hnd, name, wq_thread, wq, &wq_stack, worker_priority);
	if (rv != SYS_OK) 
	{
		wq_sem_deinit(wq);
		os_mem_free(wq);
...
static void dump_wq_info(wq_t *wq, bool verbose)
{
	...
	list_for_each_entry(job_info, wq_job_info_t, &wq->wq_job_list,
...
static void workq_debug_dump_cmd(int argc, char **argv)
{
	wq_t *wq;
	int cnt = 0;
	list_for_each_entry(wq, wq_t, &wq_list, node, struct list_head) {
