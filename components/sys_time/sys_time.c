 ...
 * Set date and time
 */
int sys_time_time_set(const struct tm *tm)
{
	time_t time;
	...
	if (validate_date_time(tm) == 0) 
	{
		...
		return sys_time_time_set_posix(time);
	} 
	else 
	{
		return -1;
 ...
 * Get date and time
 */
int sys_time_time_get(struct tm *tm)
{
	time_t curtime;
	curtime = sys_time_time_get_posix();
...
int sys_time_time_set_posix(time_t time)
{
	return rtc_time_set(time);
}

time_t sys_time_time_get_posix(void)
{
	return rtc_time_get();
}
...
bool is_sys_time_init_done(void)
{
	return init_done;
}
...
int gettimeofday(struct timeval *tv, void *tz)
{
	...
	tv->tv_sec = sys_time_time_get_posix();
