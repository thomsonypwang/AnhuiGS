...
#define psm_is_data_len_sane(__psm__, __len__) ((__len__ < OBJECT_TYPE_SMALL_BINARY_MAX_SIZE) && (__len__ < (part_size(psm) / 2)))
...
void psm_dump_hex(const char *buf, uint32_t size)
{
    ...
    for (index = 0; index < size; index++)
    {
        if (isprint((unsigned char)buf[index]))
            printf("%2x-(%c) ", (unsigned char)buf[index], buf[index]);
        else
            printf("%2x-( ) ", (unsigned char)buf[index]);
        if (!((index + 1) % 4))
            printf("\r\n");
    }
    printf("\r\n-----------------\r\n");
...
static int psm_erase_flash(psm_t *psm, uint32_t offset, uint32_t size)
{
    ...
    int rv           = flash_drv_erase(address, size);
    if (rv != SYS_OK)
    {
...
int psm_read_flash(psm_t *psm, void *buffer, uint32_t size, uint32_t offset)
{
    ...
    int rv = flash_drv_read(address, buffer, size);
    if (rv != SYS_OK)
    {
...
static int psm_write_flash_same_device_rw(psm_t *psm, const void *buffer, uint32_t size, uint32_t address)
{
    ...
    while (remaining_size)
    {
        ...
        rv = flash_drv_write(address, tmpbuf, chunk_size);
        if (rv != SYS_OK)
        {
...
static int psm_write_flash(psm_t *psm, const void *buffer, uint32_t size, uint32_t offset, bool verify)
{
    ...
    rv = flash_drv_write(address, (void *)buffer, size);
    if (rv != SYS_OK)
    {
        if (rv == SYS_E_FAULT)
...
/*
 * Return SYS_FAIL if already set. Set bit and return SYS_OK
 * otherwise.
 */
